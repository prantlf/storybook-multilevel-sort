...
const compareTypes = (type1, type2, { typeOrder = ['docs', 'story'] } = {}) => {
  ...
  if (index1 >= 0) {
    ...
    if (index2 >= 0) return index1 === index2 ? 0 : index1 < index2 ? -1 : 1
...
export const compareStories = (order, story1, story2, { typeOrder, compareNames } = {}) => {
...
export const storySort = (story1, story2) => {
  ...
  // The parameter `storyOrder` is mandatory.
  if (!storyOrder) throw new Error('Missing storyOrder in storybook-multilevel-sort:shared object. Forgot to pass it to configureSort?')
...
export const configureSort = ({ storyOrder, typeOrder, compareNames }) => {
  ...
  if (storyOrder) params.storyOrder = storyOrder
  if (typeOrder !== undefined) params.typeOrder = typeOrder
  if (compareNames) params.compareNames = compareNames
